...
class HistorikkViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let history = historyDao.createNewHistory(antallDrink: 10, antallOl: 10, antallShot: 10, antallVin: 10, dato: NSDate(), datoTwo: "", endOfSesDato: NSDate(), firstUnitTimeStamp: NSDate(), forbruk: 4000, hoyestePromille: 2.0, sessionNumber: 100, plannedNrUnits: 88)
        historyDao.save()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if histories.count == 0 {
            ...
            let dateUtil = DateUtil()
            cell.dateLabel?.text = dateUtil.getDateOfMonth(currentDate as Date?)
            cell.undertitleLabel?.text = "\(dateUtil.getDayOfWeekAsString(currentDate as Date?)!) brukte du \(currentHistory.forbruk as! Int),-"
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        //print(histories[0].sessionNumber)
        print(histories[indexPath.row].sessionNumber)
    ...
    func fetchSections(){
        for history in histories {
            ...
            let sectionLabel = DateUtil().getMonthOfYear(history.dato as Date?)! + " - " + String(describing: year)
