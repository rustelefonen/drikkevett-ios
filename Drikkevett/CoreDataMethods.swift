...
class CoreDataMethods
{
    ...
    func deleteCellGraphHistory(histSessionNr: Int) {
        ...
        do {
            ...
            if let fetchResults = try moc.executeFetchRequest(fetchRequest) as? [GraphHistorikk] {
                ...
                for items in graphItems {
                    let tempSesNr = items.sessionNumber
                    var tempIndex = 0
                    if let index = graphItems.indexOf(items) {
                        tempIndex = index
                    }
                    if(tempSesNr == histSessionNr){
                        moc.deleteObject(graphItems[tempIndex])
                    }
                }
            }
            do {
    ...
    func deleteLastUnitAdded(unit:String){
        var timeStamp = [TimeStamp2]()
        var tempNr = 0
        let fetchRequest = NSFetchRequest(entityName: "TimeStamp2")
        do {
            if let fetchResults = try moc.executeFetchRequest(fetchRequest) as? [TimeStamp2] {
                timeStamp = fetchResults
                for units in timeStamp {
                    if(units.unitAlkohol == unit){
                        if let index = timeStamp.indexOf(units) {
                            if(index > tempNr){
                                tempNr = index
                            }
                        }
                ...
                moc.deleteObject(timeStamp[tempNr])
            }
            do {
                try moc.save()
            } catch {
