...
class FormController: UITableViewController, UITextFieldDelegate, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let green = UIColor(red: 112/255, green: 221/255, blue: 127/255, alpha: 255/255)
        ageInputField.attributedPlaceholder = NSAttributedString(string:"Velg din alder...", attributes:[NSForegroundColorAttributeName: green])
        genderInputField.attributedPlaceholder = NSAttributedString(string:"Velg ditt kjønn...", attributes:[NSForegroundColorAttributeName: green])
        countyInputField.attributedPlaceholder = NSAttributedString(string:"Velg ditt fylke...", attributes:[NSForegroundColorAttributeName: green])
        titleInputField.attributedPlaceholder = NSAttributedString(string:"Skriv tittel...", attributes:[NSForegroundColorAttributeName: green])
    ...
    private func presentValidationAlert(title:String, content:String) {
        ...
        presentViewController(alert, animated: true, completion: {() -> Void in
            self.tableView.deselectRowAtIndexPath(NSIndexPath(forItem: 0, inSection: 3), animated: true)
        })
    }
    ...
    private func onResponse(data:NSData?, loadingDialog:UIAlertController){
        ...
        if data == nil {
            return showErrorOccouredDialog()
        } else {
